...
The response should be in **markdown format**, and use appropriate **icons** for:
- ⚠️ / 🐞 → Bugs
...
Evaluate in terms of code quality, security, performance, and best practices.${customPromptSection}
`;
    }
...
`;
    ...
    /**
     * Claude sağlayıcısı için özel prompt oluşturur
     * @param diff Git diff içeriği
     * @param languageId Programlama dili
     * @returns Claude için özelleştirilmiş prompt
     */
    static createClaudeReviewPrompt(diff: string, languageId: string): string {
        const customPrompt = ConfigurationManager.getCustomPrompt();
        const customPromptSection = customPrompt ? `\n\nAdditional Instructions:\n${customPrompt}` : '';
